...
def pool_measure_parallel(WF, pool_index, H, pool_data, theta):

    excitation = pool_data["excitation indeces"][pool_index]
    excitation_type = pool_data["excitation type"][pool_index]

    pool_WF = copy.deepcopy(WF)
    pool_WF.ups_layout.excitation_indices.append(np.array(excitation)-pool_WF.num_inactive_spin_orbs)
    pool_WF.ups_layout.excitation_operator_type.append(excitation_type)
    pool_WF.ups_layout.n_params += 1
    pool_WF._thetas.append(theta)
    pool_WF.ci_coeffs = construct_ups_state(pool_WF.ci_coeffs, pool_WF.ci_info, pool_WF.thetas, pool_WF.ups_layout)

    energy = float(expectation_value(pool_WF.ci_coeffs, [H], pool_WF.ci_coeffs, pool_WF.ci_info, pool_WF.thetas, pool_WF.ups_layout))

    return energy

def optimizer_pool_parallel(thetas, energies):
    '''
    Solves systems of equations for coefficients of energy landscape
    ...
    energy_eval = [energies[i:i+5] for i in range(0, len(energies), 5)]
    theta_eval = [thetas[i:i+5] for i in range(0, len(thetas), 5)]
    theta_min_pool = []
    energy_min_pool = []

    for en_sample, theta_sample in zip(energy_eval, theta_eval):

        thetas = np.array(theta_sample)
        energies = np.array(en_sample)
        ...
        theta_min, energy_min = global_min_search(A, B, C, D, E)
        theta_min_pool.append(theta_min)
        energy_min_pool.append(energy_min)

    return theta_min_pool, energy_min_pool
...
def rotoadapt_2(WF, H, pool_data, max_epochs = 20, adapt_threshold = 1e-5, opt_threshold = 1e-5):
    '''
    ...
    # Define parameters for pool measurement parallelization
    theta_sample = [(2*np.pi*l)/5.5 for l in range(0,5)]  # thetas for evaluations of each pool candidate
    theta_sample_pool = [theta for _ in range(len(excitation_pool)) for theta in theta_sample]
    pool_index_theta = list(itertools.chain.from_iterable(itertools.repeat(i, 5) for i in range(len(excitation_pool))))
    ...
    while converged == False:
        ...
        with mp.Pool(processes=os.cpu_count()) as pool:
            energies_sample_pool = pool.starmap(pool_measure_parallel, [(WF, pool_index_theta[i], H, pool_data, theta_sample_pool[i]) for i in range(len(theta_sample_pool))])

        theta_pool, energy_pool = optimizer_pool_parallel(theta_sample_pool, energies_sample_pool)
        print(energy_pool)
